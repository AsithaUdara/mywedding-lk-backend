...
public class ApplyPersonalizedChecklistCommandHandler
    : IRequestHandler<ApplyPersonalizedChecklistCommand, ApplyPersonalizedChecklistResult>
{
    ...
    public ApplyPersonalizedChecklistCommandHandler(
    ...
    public async Task<ApplyPersonalizedChecklistResult> Handle(
        ApplyPersonalizedChecklistCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (!canManage)
        {
            throw new ForbiddenAccessException("You do not have permission to generate the checklist for this event.");
        }

        if (weddingEvent.TaskPlanPhase == TaskPlanPhase.Full)
        {
            return new ApplyPersonalizedChecklistResult(
                0,
                TaskPlanPhase.Full.ToString(),
                "Master checklist was already generated for this event.");
        }

        var excludeSet = new HashSet<string>(request.ExcludeTemplateTitles, StringComparer.OrdinalIgnoreCase);
        var created = await WeddingChecklistMaterializer.MaterializeFullChecklistAsync(
            request.EventId,
            weddingEvent.EventDate,
            _taskRepository,
            excludeSet,
            request.AdditionalTasks,
            cancellationToken);
        ...
        return new ApplyPersonalizedChecklistResult(
            created,
            TaskPlanPhase.Full.ToString(),
            $"Generated {created} tasks on the master checklist.");
    }
}
